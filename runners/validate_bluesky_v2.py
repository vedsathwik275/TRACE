# validate_bluesky_v2.py
"""
Quality assurance validation script for TRACE Bluesky Scraper V2 output.
...
def find_latest_v2_csv() -> str | None:
    """
    Find the most recent CSV file in data/ starting with 'trace_bluesky_v2'.

    Returns:
        Full path to the file, or None if not found.
    """
    pattern = "data/trace_bluesky_v2*.csv"
...
def check_record_count(df: pd.DataFrame) -> tuple[bool, str]:
    """
    Check 1: Verify total rows are at least 100.
    Warn (but not fail) if under 500.
    """
    count = len(df)
    if count >= 500:
    ...
    return False, f"FAIL ({count} rows - minimum 100 required)"


def check_achilles_rate(df: pd.DataFrame) -> tuple[bool, str]:
    """
    Check 2: Verify at least 30% of records have is_achilles_related == True.
    Bluesky is a newer platform, may have less historical Achilles data.
    """
    ...
    return False, f"FAIL ({rate:.1f}% achilles-related - need ≥30%)"


def check_relevance_threshold(df: pd.DataFrame) -> tuple[bool, str]:
    """
    Check 3: Verify every row has relevance_score >= 1.0 (BLUESKY_RELEVANCE_THRESHOLD).
    Show count of failing rows and 3 worst offenders if any fail.
    """
    if "relevance_score" not in df.columns:
        return False, "FAIL (column missing)"

    failing = df[df["relevance_score"] < 1.0]
    failing_count = len(failing)

    if failing_count == 0:
        min_score = df["relevance_score"].min()
        return True, f"PASS (all scores ≥ 1.0, min = {min_score:.2f})"
...
def check_temporal_coverage(df: pd.DataFrame) -> tuple[bool, str]:
    """
    Check 4: Verify records span at least 2 distinct calendar years.
    Bluesky launched in 2022, so less historical coverage is expected.
    """
    ...
    return False, f"FAIL (only {unique_years} distinct years - need ≥2)"


def check_player_coverage(df: pd.DataFrame) -> tuple[bool, str]:
    """
    Check 5: Parse mentioned_players JSON, verify at least 3 distinct players.
    """
    ...
    return False, f"FAIL (only {unique_count} unique players - need ≥3)"
...
def main() -> None:
    ...
    """
    print("=" * 60)
    print("🔍 TRACE BLUESKY SCRAPER V2 - DATA VALIDATION")
    print("=" * 60)
    ...
    if csv_path is None:
        print("\n❌ ERROR: No CSV file found matching 'data/trace_bluesky_v2*.csv'")
        print("   Please run runners/bs_run_v2.py first to generate data.")
        sys.exit(1)
